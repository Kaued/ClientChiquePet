...
export const SingleOrder = () => {
  ...
  useEffect(() => {
    if (order.data) {
      setStatus(
        Config.statusOrder.find((a) => a.statusId == order.data!.statusOrder)!,
      );
    }
  }, [order.data]);
        ...
        <>
          <Flex className="order-content">
            ...
            <Flex className="order-content__dateTotal">
              <Heading>
                {new Date(order.data.createDate).toLocaleDateString("pt-BR")}
              </Heading>
              <Text>
                <span>R$</span>
              ...
              order.data.orderProducts.map((item) => (
                ...
                >
                  <Flex className="order-product">
                    <Image
                      src={`${Config.baseApiUrl}/${item.product.imageUrl.find(
                        (f) => f.type == "cover",
                      )?.path}`}
