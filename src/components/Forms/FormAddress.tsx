...
export const FormAddress = ({ isAddMode, addressValues, submit, open }: FormAddressData) => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const title: string = isAddMode ? 'Adicionar novo endereço' : 'Alterar endereço';
  ...
  const formik = useFormik<AddressData>({
    ...
    validationSchema: Yup.object().shape({
      cep: Yup.string().required('O campo cep é obrigatório').length(8, 'O cep deve ter 8 numeros'),
  ...
  return (
    <Flex>
      {isAddMode && (
        <Button colorScheme="green" onClick={onOpen} className="form-address__button">
          <FaPlus /> Adicionar
        </Button>
      )}
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button colorScheme="green" onClick={() => formik.handleSubmit()} className="form-address__button--bottom">
              <FaRegSave />
              Salvar
            </Button>
          </ModalFooter>
